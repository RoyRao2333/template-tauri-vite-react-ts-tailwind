...
async function locateAppContext(): Promise<AppContext> {
  ...
  const context = candidates.find((candidate): candidate is AppContext => candidate !== null);
  if (!context) {
...
async function updateCargoToml(filePath: string, packageName: string, authors: string[], libName: string): Promise<void> {
...
async function updateMainRs(filePath: string, oldLibName: string, newLibName: string): Promise<void> {
  const original = await readFile(filePath, 'utf8');
...
async function updateTextFile(filePath: string, from: string, to: string): Promise<void> {
  const original = await readFile(filePath, 'utf8');
...
async function main(): Promise<void> {
  ...
  const currentLibNameMatch = currentCargo.match(/\[lib\][\s\S]*?^\s*name = "([^"]+)"\s*$/m);
  if (!currentLibNameMatch) {
    throw new Error(`Cannot read current Rust lib name from ${context.cargoTomlPath}`);
  }
  const currentLibName = currentLibNameMatch[1];

  const newAppDir = path.join(path.dirname(context.appDir), packageName);
  const newAppPackageName = `@app/${packageName}`;
  ...
  await updateCargoToml(context.cargoTomlPath, packageName, ['example'], newLibName);
  await updateMainRs(context.mainRsPath, currentLibName, newLibName);
  await updateTextFile(context.appReadmePath, `apps/${currentProjectName}`, `apps/${packageName}`);
